	...
	public TarjetaMiBecaDTO consultaBeneficiario(RegistroDTO registro)
		...
		try {
			...
			if (response.getStatus() == 200) {
				 ...
				 */
				LOGGER.warn("El Webservice de Sistema Microcreditos nos devolvió un código: " + response.getStatus()
						+ " Esta fue la respuesta obtenida: " + respuesta);
				throw new SistemaMicrocreditosException("msj_datos_incorrectos_suac");
			} else {
				LOGGER.error("El Webservice de Sistema Microcreditos nos devolvió un código de error: " + response.getStatus()
						+ " Esta fue la respuesta obtenida: " + respuesta);
			...
			LOGGER.error("No se pudo establecer la conexión al servicio web de Sistema Microcreditos: ", e);
			throw new SistemaMicrocreditosException("msj_no_conexion_suac");
		} catch (ClientHandlerException ex) {
			LOGGER.error("Error al realizar la conexión con el servicio WEB de Sistema Microcreditos: ", ex);
			throw new SistemaMicrocreditosException("msj_no_conexion_suac");
