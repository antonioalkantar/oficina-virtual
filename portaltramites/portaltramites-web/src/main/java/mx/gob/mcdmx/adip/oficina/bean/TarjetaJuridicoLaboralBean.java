...
public class TarjetaJuridicoLaboralBean implements Serializable {
	...
	public TarjetaJuridicoLaboralBean() {}
	...
	public void init() {
		try {
			...
			registroVentanillaUnicaConstr.setCatSistemaDTO(new CatSistemaDTO(tramitesOficinaBean.getID_SISTEMA_JURIDICO_LABORAL()));
	...
	public String urlAction() {
		if (registroVentanillaUnicaConstr != null) {
			if (Environment.getAppProfile().compareTo("dev") == 0
				...
				return Constantes.RETURN_URL_JURIDICO_LABORAL_DEV + registroVentanillaUnicaConstr.getIdRegistroOrigen();
			} else {
				return Constantes.RETURN_URL_JURIDICO_LABORAL_PROD + registroVentanillaUnicaConstr.getIdRegistroOrigen();
			}
		}
		return "#";
	}
